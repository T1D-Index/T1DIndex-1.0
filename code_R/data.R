...
get_complication_parameters <- function() {
  ...
  const <- Data_Run_query_return_df( 'SELECT complication, hba1c_lt_8, hba1c_8_to_9, hba1c_gteq_9
    FROM index_parameters.constant_complications;')
  
  
  weib <- Data_Run_query_return_df(  'SELECT abbrev, intercept, slope, scale, name, graph_title
    FROM index_parameters.weibull_complications;')
...
get_disease_weights <- function() {
  ...
  dw  <- Data_Run_query_return_df(   'SELECT complication, "DALYs" FROM index_parameters.disease_weights;')
